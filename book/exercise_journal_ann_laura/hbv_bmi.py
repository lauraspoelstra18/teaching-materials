    ...
    def update(self) -> None:
            ...
            if self.current_timestep < self.end_timestep:
                ...
                if self.P_dt > 0:
                    ...
                    self.Pe_dt = max(0, self.Si - self.Imax) # adjust if needed
                    self.Si    = min(self.Si, self.Imax) # adjust if needed
                ...
                if self.Pe_dt > 0:
                    cr       = (self.Su / self.Sumax) ** self.beta # adjust if needed
                    cr = min(max(cr, 0), 1)
                ...
                self.Ea_dt = (self.Su / (self.Sumax * self.Ce)) * self.Ep_dt # adjust if needed
                self.Ea_dt = min(self.Ea_dt, self.Su) # adjust if needed
                self.Su    = self.Su - self.Ea_dt # adjust if needed

                # Percolation
                self.Qus_dt = self.Pmax * (self.Su / self.Sumax) # adjust if needed
